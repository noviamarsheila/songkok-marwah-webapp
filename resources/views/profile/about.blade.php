@extends('profile.layouts.main')

@section('content')
<!------------------- VISI MISI ------------------------>
<div class="container visimisi">
    <div class="d-flex align-items-center mb-3">
        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="rgb(77 124 15)" class="bi bi-chat-left-quote-fill" viewBox="0 0 16 16">
            <path
                d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H4.414a1 1 0 0 0-.707.293L.854 15.146A.5.5 0 0 1 0 14.793zm7.194 2.766a1.7 1.7 0 0 0-.227-.272 1.5 1.5 0 0 0-.469-.324l-.008-.004A1.8 1.8 0 0 0 5.734 4C4.776 4 4 4.746 4 5.667c0 .92.776 1.666 1.734 1.666.343 0 .662-.095.931-.26-.137.389-.39.804-.81 1.22a.405.405 0 0 0 .011.59c.173.16.447.155.614-.01 1.334-1.329 1.37-2.758.941-3.706a2.5 2.5 0 0 0-.227-.4zM11 7.073c-.136.389-.39.804-.81 1.22a.405.405 0 0 0 .012.59c.172.16.446.155.613-.01 1.334-1.329 1.37-2.758.942-3.706a2.5 2.5 0 0 0-.228-.4 1.7 1.7 0 0 0-.227-.273 1.5 1.5 0 0 0-.469-.324l-.008-.004A1.8 1.8 0 0 0 10.07 4c-.957 0-1.734.746-1.734 1.667 0 .92.777 1.666 1.734 1.666.343 0 .662-.095.931-.26z"
            />
        </svg>
        <h2 class="title-section ms-2">Visi Misi</h2>
    </div>
    <div class="row">
        <div class="col-lg-6">
            <h1 class="fs-1 fw-bold">Visi</h1>
            <ul>
                @foreach ($visi as $item)
                <li>
                    <p class="headline-paragraph text-black-50">{{ $item->deskripsi }}</p>
                </li>
                @endforeach
            </ul>
        </div>
        <div class="col-lg-6">
            <h1 class="fs-1 fw-bold">Misi</h1>
            <ul>
                @foreach ($misi as $item)
                <li>
                    <p class="headline-paragraph text-black-50">{{ $item->deskripsi }}</p>
                </li>
                @endforeach
            </ul>
        </div>
    </div>
</div>
<!------------------- END VISI MISI ------------------------>

<!------------------- PILIH KAMI ------------------------>
<section id="whychooseus">
    <div class="container my-3">
        <h2 class="title-section text-center">Mengapa Memilih Kami</h2>
        <div class="row mt-4 gap-3 gap-lg-0 gy-0 gy-lg-4">
            @foreach ($pilihkami as $item)
            <div class="col-xl-4 col-lg-6">
                <div class="d-flex align-items-center shadow-sm --box-whychooseus">
                    <img src="images/about/check_icon.png" alt="" width="45" height="45" />
                    <p class="headline-paragraph text-black-50 ms-2">{{ $item->deskripsi }}</p>
                </div>
            </div>
            @endforeach
        </div>
    </div>
</section>

<!------------------- END PILIH KAMI ------------------------>
@endsection
